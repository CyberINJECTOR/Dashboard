...
let DeletePopupTaskComponent = class DeletePopupTaskComponent {
    ...
    ngOnInit() {
        this.taskService.getObservableValue().subscribe((entityUpdated) => {
...
class EntityBase {
    SortArrayEntities(arrayEntities, entityToSort) {
...
let HomeComponent = class HomeComponent {
    ...
    ngOnInit() {
        this.taskService.getObservableValue().subscribe((entitySaved) => {
            ...
            if (this.newEntityToSave.accion === _Entities_Accion_enum__WEBPACK_IMPORTED_MODULE_7__["Accion"].Save) {
                (entitySaved.entity === _Entities_Entity_insert_model__WEBPACK_IMPORTED_MODULE_8__["Entity"].Task) ? this.taskArrayList.push(entitySaved) : this.noteArrayList.push(entitySaved);
    ...
    sortTaskArray(taskArray) {
        const entityToReplace = taskArray.find(x => x.id === this.newEntityToSave.id);
        const index = taskArray.indexOf(entityToReplace);
    ...
    sortNoteArray(noteArray) {
        const entityToReplace = noteArray.find(x => x.id === this.newEntityToSave.id);
        const index = noteArray.indexOf(entityToReplace);
